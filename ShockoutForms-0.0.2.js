       ...
       */
            ...
            try {
                ...
                self.$form.find('[data-bind]').each(function (i, e) {
                    if (rxIncludeInputTags.test(e.tagName) && !rxExcludeInputTypes.test($(e).attr('type')) || $(e).attr('contenteditable') == 'true') {
                        var key = Shockout.Utils.observableNameFromControl(e, self.viewModel);
        ...
        */
        SPForm.prototype.bindListItemValues = function (self) {
            ...
            try {
                ...
                Shockout.SpApi.executeRestRequest(item.CreatedBy.__deferred.uri, function (data, status, jqXhr) {
                    var person = data.d;
                    vm.CreatedBy(person);
                ...
                Shockout.SpApi.executeRestRequest(item.ModifiedBy.__deferred.uri, function (data, status, jqXhr) {
                    var person = data.d;
        ...
        */
        SPForm.prototype.getAttachments = function (self, callback) {
            ...
            var attachments = [];
            Shockout.SpApi.executeRestRequest(self.listItem.Attachments.__deferred.uri, function (data, status, jqXhr) {
                try {
        ...
        SPForm.prototype.formIsValid = function (model, showDialog) {
            ...
            try {
                self.$form.find('.required, [required]').each(function checkRequired(i, n) {
                    var p = Shockout.Utils.observableNameFromControl(n, self.viewModel);
                    if (!!p && model[p]) {
                        var val = model[p]();
...
(function (Shockout) {
    var SpSoap = (function () {
        ...
        * @return void
        */
        SpSoap.executeSoapRequest = function (action, packet, params, siteUrl, callback) {
            if (siteUrl === void 0) { siteUrl = '/'; }
            if (callback === void 0) { callback = undefined; }
            siteUrl = Shockout.Utils.formatSubsiteUrl(siteUrl);
            try {
                var serviceUrl = siteUrl + '_vti_bin/lists.asmx';
                if (params != null) {
    ...
    var Utils = (function () {
        ...
        */
        Utils.observableNameFromControl = function (control, vm) {
            ...
            var rx = /(\b:\s*|\$root\.|\$data|\$parent)\w*\b/;
            ...
            if (/\$data/.test(result) || /\$parent/.test(result)) {
                var parent = Utils.getParent(control);
                var koName;
                //First, look for `<!-- ko foreach: MyVar -->` comment syntax, then try closest parent element.
                var foreachComment = Utils.getPrevKOComment(control);
                if (!!foreachComment) {
                    koName = foreachComment.textContent.replace(/\s*ko\s*foreach\s*:\s*(\$root\.|)/, '');
                }
                //var fe: any = /<!--\s*ko\s+foreach:\s*(\$root\.|)\w*\s*(-->|)/.exec(parent.innerHTML);
                //fe = !!fe ? fe[0]
                //    .replace(/<!--\s*ko\s*foreach\s*:\s*(\$root\.|)/, '')
                //    .replace(/\s*-->/, '') : null;
                // Check against view model if passed as param. Return if koName in view model or found. Otherwise keep searching for the next closest parent.
                return vm && vm[koName] ? koName : !!koName ? koName : Utils.observableNameFromControl(parent);
            }
            return result;
        };
