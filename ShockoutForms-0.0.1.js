    ...
    var SPForm = (function () {
        function SPForm(listName, formId, options) {
            ...
            */
            ...
            this.executeRequest = function (action, packet, params, callback, serviceUrl) {
                if (callback === void 0) { callback = undefined; }
                if (serviceUrl === void 0) { serviceUrl = this.rootUrl + this.siteUrl + '/_vti_bin/lists.asmx'; }
                try {
                    if (params != null) {
        ...
        */
        ...
        SPForm.prototype.bindObservable = function (key, val, self) {
            ...
            if (key in self.viewModel) {
                ...
                self.viewModel[key] = val != null && /Array/.test(val.constructor) ? ko.observableArray(val) : ko.observable(val);
            }
        };
        /**
            ...
            try {
                ...
                self.getListItemsRest(self.listItem.Attachments.__deferred.uri, function (data, status, jqXhr) {
                    $.each(data.d.results, function (i, att) {
                        attachments.push(new Shockout.Attachment(att));
                    });
...
(function (Shockout) {
    var Templates = (function () {
        ...
        Templates.getFileUploadTemplate = function () {
            var $div = $('<div>').html('<div class="qq-uploader">' + '<div class="qq-upload-drop-area"><span>Drop files here to upload</span></div>' + '<div class="btn btn-primary qq-upload-button"><span class="glyphicon glyphicon-paperclip"></span> Attach File</div>' + '<ul class="qq-upload-list"></ul></div>');
            return $div.html();
        };
        Templates.getCreatedModifiedHtml = function () {
            var template = '<div class="create-mod-info no-print"></div>' + '<div class="row">' + '<div class="col-md-3"><label>Created By</label> <a data-bind="text: {0}, attr:{href: \'mailto:\'+{1}()}" class="email" > </a></div>' + '<div class="col-md-3"><label>Created</label> <span data-bind="spDateTime: {2}"></span></div>' + '<div class="col-md-3"><label>Modified By</label> <a data-bind="text: {3}, attr:{href: \'mailto:\'+{4}()}" class="email"></a></div>' + '<div class="col-md-3"><label>Modified</label> <span data-bind="spDateTime: {5}"></span></div>' + '</div>';
            template = template.replace(/\{0\}/g, Shockout.ViewModel.createdByKey).replace(/\{1\}/g, Shockout.ViewModel.createdByEmailKey).replace(/\{2\}/g, Shockout.ViewModel.createdKey).replace(/\{3\}/g, Shockout.ViewModel.modifiedByKey).replace(/\{4\}/g, Shockout.ViewModel.modifiedByEmailKey).replace(/\{5\}/g, Shockout.ViewModel.modifiedKey);
            var $div = $('<div>').html(template);
            return $div.html();
        };
        Templates.getHistoryTemplate = function () {
            var template = '<h4>Workflow History</h4>' + '<table border="1" cellpadding="5" cellspacing="0" class="data-table" style="width:100%;border-collapse:collapse;">' + '<thead>' + '<tr><th>Description</th><th>Date</th></tr>' + '</thead>' + '<tbody data-bind="foreach: {0}">' + '<tr><td data-bind="text: {1}"></td><td data-bind="text: {2}"></td></tr>' + '</tbody>' + '</table>'.replace(/\{0\}/g, Shockout.ViewModel.historyKey).replace(/\{1\}/g, Shockout.ViewModel.historyDescriptionKey).replace(/\{2\}/g, Shockout.ViewModel.historyDateKey);
            var $div = $('<div>', {
                'data-bind': 'visible: {0}().length > 0'.replace(/\{0\}/i, Shockout.ViewModel.historyKey)
            });
            return $div;
        };
        ...
        Templates.getAttachmentsTemplate = function (fileuploaderId) {
            var template = '<h4>Attachments (<span data-bind="text: attachments().length"></span>)</h4>' + '<div id="' + fileuploaderId + '"></div>' + '<div data-bind="visible: attachments().length > 0">' + '<table class="attachments-table">' + '<tbody data-bind="foreach: attachments">' + '<tr>' + '<td><a href="" data-bind="text: title, attr: {href: href, \'class\': ext}"></a></td>' + '<td><button data-bind="event: {click: $root.deleteAttachment}" class="btn del" title="Delete"><span class="glyphicon glyphicon-remove"></span><span>Delete</span></button></td>' + '</tr>' + '</tbody>' + '</table>' + '</div>';
            var $div = $('<div>', { 'html': template });
            return $div;
        };
